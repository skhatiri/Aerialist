...
def copy_files_volume(self):
    ...
    file_helper.create_dir(local_folder)

    # handle drone files
    if self.config.drone is not None:
        if self.config.drone.mission_file is not None:
            container_config.drone.mission_file = file_helper.copy_local(
                self.config.drone.mission_file, local_folder
            )
        if (
            self.config.drone.params is None
            and self.config.drone.params_file is not None
        ):
            container_config.drone.params_file = file_helper.copy_local(
                self.config.drone.params_file, local_folder
            )

    # handle mission commands
    if self.config.mission is not None:
        self.config.mission.save_commands_list_if_needed(local_folder)
        if (
            self.config.mission.commands_file is not None
            and self.config.mission.commands is not None
            and len(self.config.mission.commands)
            ...
            container_config.mission.commands_file = file_helper.copy_local(
                self.config.mission.commands_file, local_folder
            )

    # Assertion configs skipped (same as cloud version)

    if container_config.agent is not None:
        container_config.agent.engine = AgentConfig.LOCAL
        container_config.agent.count = 1

    # Write the YAML into the mounted volume
    self.container_test_yaml = container_config.to_yaml(
        f"{local_folder}{self.config.agent.id}.yaml"
    )

    logger.info(f"files prepared in host volume: {local_folder}")
    return container_config
