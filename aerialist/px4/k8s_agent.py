...
class K8sAgent(DockerAgent):
    ...
    # KUBE_CMD = 'yq ".metadata.name = \"${name}\" \
    #     | .spec.template.spec.containers[0].env |= map(select(.name == \"COMMAND\").value = \"${command}\") \
    #     | .spec.completions = ${runs} \
    #     | .spec.parallelism = ${runs} \
    #     | .spec.template.spec.containers[] | select(.name == \"sim\").image = \"${image}\"" ${template} | kubectl apply -f - --validate=true'
